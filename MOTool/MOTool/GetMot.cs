...
public static class GetMot
{
    public static void processMot(string file)
    {
        ...
        using (BinaryReader br = new BinaryReader(fs))
        {
            ...
            byte countOfKeyframedproperties = GlobalTools.readValue(fs, br, 6, GlobalTools.NumberType.BYTE);
            byte UseLoop = GlobalTools.readValue(fs, br, 7, GlobalTools.NumberType.BYTE);
            fs.Position = 8;
            GlobalTools.changeColor(ConsoleColor.Green);
            ...
            for (int i = 8; i < propertyEndOffset; i += 12)
            {
                ...
                if (GlobalTools.readValue(fs, br, i + 8, GlobalTools.NumberType.UINT) != 0 && GlobalTools.readValue(fs, br, i + 8, GlobalTools.NumberType.UINT) < fs.Length && GlobalTools.readValue(fs, br, i, GlobalTools.NumberType.UBYTE) != 127)
                {
                    ...
                    bone.type = GlobalTools.getType(GlobalTools.readValue(fs, br, i + 1, GlobalTools.NumberType.BYTE));
                    bone.typeOriginal = GlobalTools.readValue(fs, br, i + 1, GlobalTools.NumberType.BYTE);
                    ...
                    Console.WriteLine(bone.type + " " + bone.index + " " + bone.typeOriginal);
            ...
            {
                if (MOTFile.bones[i].typeOriginal < 80)
                {
                    int additive = 0;
                ...
                {
                    for (uint a = MOTFile.bones[i].data.adress; a < (((MOTFile.bones[i].data.adress + (MOTFile.bones[i].data.keyframeCount * 16)))); a += 16)
                    {
                        if (a == fs.Length) break;
                        MOTKeyframe cv = new MOTKeyframe(); //curve
                        cv.absolute_index = GlobalTools.readValue(fs, br, a, GlobalTools.NumberType.USHORT);
                        cv.P0 = GlobalTools.readValue(fs, br, a + 4, GlobalTools.NumberType.SINGLE);
                        cv.M0 = GlobalTools.readValue(fs, br, a + 8, GlobalTools.NumberType.SINGLE);
                        cv.M1 = GlobalTools.readValue(fs, br, a + 12, GlobalTools.NumberType.SINGLE);
                        Console.WriteLine(a + " " + cv.absolute_index + " " + MOTFile.bones[i].index + "   " + cv.P0);
                        MOTFile.addCurve(cv, MOTFile.bones[i].data);
                    }
                }
            }
