...
class OBJECT_OT_Export(Operator):
    ...
    def execute(self, context):
        ...
        #os.system('start cmd /k ' + context.scene.exe_path + ' ' + file_path + ' ' + action.name + ' ' + str(context.scene.loop) + ' ' + str(context.scene.bones_that_uses_ik) + ' ' + str(context.scene.bone_path) + ' ' + str(context.scene.output))
    ...
    def draw(self, context):
        ...
        if armature_obj and armature_obj.type == 'ARMATURE':
            ...
            def print_bone_info(bone, level=0):
                ...
                VIEW3D_PT_Panel.count += 1
                print(f"{bone.name}: parent: {parent_index} level: {VIEW3D_PT_Panel.count-1}")
                boxHI.label(text=f"Bone:{bone.name}           Level: {VIEW3D_PT_Panel.count-2}");
                boxHI.scale_y = 0.5
